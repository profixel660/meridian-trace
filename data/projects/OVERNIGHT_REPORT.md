...
### Round 17 — Setup wizard + FastAPI sidecar

Round 17 lands the user-facing setup wizard and the Tauri-side glue that makes the desktop app self-contained. Path A round 17 per `docs/DECISIONS.md` §3.7 / §10. Four parallel streams, file-disjoint; the actual `.msi` still awaits round 18's Rust install. The intent of round 17 is "every line of code that produces the wizard UX is on disk and unit-testable, so round 18 reduces to `rustup install` + a clean `tauri:build`".

- **Stream A — Tauri sidecar wiring** (`src-tauri/src/lib.rs`, `src-tauri/Cargo.toml`, `src-tauri/tauri.conf.json`, `src-tauri/capabilities/default.json`). Replaces the round-16 `// ROUND-17: spawn FastAPI sidecar here` marker with the real spawn pattern. Primary path: `tauri_plugin_shell::ShellExt::shell().sidecar("meridian-server")` — picks up the bundled PyInstaller binary that round 18 drops into `src-tauri/binaries/meridian-server-x86_64-pc-windows-msvc.exe`. Dev-mode fallback (round-17 state, no PyInstaller binary on disk yet): spawn `python -m uvicorn meridian.api.main:app --host 127.0.0.1 --port 8000` from `MERIDIAN_PYTHON` env-var or a `.venv/Scripts/python.exe` lookup, so a developer can `npm run tauri:dev` against the source tree once Rust is installed without first rebuilding the sidecar. The `CommandChild` handle is parked on `App::manage` and a `WindowEvent::CloseRequested` hook calls `kill()` on it before the window closes — no orphaned uvicorn processes after a quit. Window display is gated behind a TCP-connect health-check poll on `127.0.0.1:8000` (max 30s, 100ms intervals) so the wizard never renders against a backend that isn't listening yet. `tauri.conf.json` declares `bundle.externalBin: ["binaries/meridian-server"]`, priming round 18's PyInstaller drop-in. Capabilities tightened from round-16's wide-open default — `shell:allow-execute` scoped to the sidecar binary plus the `python -m uvicorn` fallback shape; `dialog:allow-open` retained for the wizard's file pickers; `fs:default` retained for state-file writes. **This round still doesn't compile** — Rust isn't installed yet, so Stream A's contract is "the source compiles cleanly when round 18 installs Rust"; round 18's first task post-install is verifying the sidecar wiring boots.

- **Stream B — wizard frontend** (`apps/web/src/app/setup/{page, api-key, first-project, first-documents, ready}/page.tsx` — five new pages plus `setup/layout.tsx`, plus `components/setup/{SetupShell, StepNav, copy.ts}`, plus `lib/{tauri, setupClient}.ts`; `app/page.tsx` modified to redirect to `/setup` when state is incomplete). Step sequence: **welcome → api-key → first-project → first-documents → ready**. WHY-before-HOW prose discipline throughout — every step opens with a one-paragraph "why this matters for your project" framing in PM language (no API-key-shaped or schema-shaped jargon) before the input controls. Three-outcome validation per step: `valid` (advance), `invalid` (block + explain in plain English), `unable_to_verify` (offer skip-with-warning rather than hard-block — e.g. API-key check fails because the user is offline). Native file pickers via Tauri's dialog API where available with a browser-`<input type="file">` fallback — bridge lives in `lib/tauri.ts` (single `pickFiles()` entry point that branches on `window.__TAURI_INTERNALS__`). Keyboard nav baked in per the UX-discoverability discipline: `?` opens a shortcut sheet on every page, `Enter` triggers the primary action, `Escape` triggers cancel-with-confirm, `Tab`-focus order matches reading order. AuthGate posture is **the wizard runs pre-auth** — `/setup/*` pages are deliberately outside `AuthGate`, since the wizard is what sets up auth. Only the final `/setup/ready` step applies AuthGate, by which point the API key is persisted and the session can mint. `setupClient.ts` types ratchet against Stream C's API contract.

- **Stream C — backend wizard API** (`src/meridian/wizard/{__init__, api, models, state}.py` new module; `src/meridian/api/main.py` registers the wizard router; `tests/e2e/test_wizard_api.py` ~10 new tests — soft until Stream C confirms the exact count). Six new endpoints: `GET /setup/state`, `POST /setup/api-key` (rate-limited 10/5min, mirrors §3.3's `/auth/login` posture), `POST /setup/projects` (409 on slug conflict), `POST /setup/import` + `GET /setup/import/{job_id}` (job-poll pattern so the wizard can render progress without holding a long request open), `POST /setup/import/skip`, `POST /setup/complete`. **All public** — no `Depends(require_session)`, deliberately, marked in source with `# DEFERRED §3.2` so the next §3.2 sweep finds them. State file is `_meridian/onboarding_state.json`, **shared with the existing CLI wizard** (`src/meridian/onboarding/wizard.py` primitives reused), so a CLI-started → GUI-finished flow (or vice-versa) is consistent. API key persistence flows through `KeyringStore` per §3.1 — never written to disk in plaintext. Ruff clean across the new module; `pytest tests/e2e/` green.

- **Stream D — docs** (this section, `docs/DECISIONS.md` round-17 follow-ups subsection at the bottom + §3.2 follow-up amendment, `apps/web/README.md` Tauri section updated to reflect round-17 actually-runnable state). Documentation-only stream; no source touched.

The four streams ran on the parallel-subagents-default discipline; file-disjoint boundaries held — Stream A owned `src-tauri/**`, Stream B owned `apps/web/src/**` + `apps/web/package.json` + `apps/web/next.config.ts`, Stream C owned `src/meridian/**` + `tests/**`, Stream D owned the three docs files. Cross-stream contract: Stream B's `setupClient.ts` types match Stream C's Pydantic models field-for-field; Stream A's spawn pattern means the wizard can call `apiFetch` against `localhost:8000` from the moment the window opens (not "configure backend first"); the wizard's pre-auth posture (no `require_session` on `/setup/*`) is the correct chicken-and-egg solution — the wizard is what sets up auth, so it can't itself require auth.

**What works without Rust today:** `npm run dev` renders the wizard if pointed at a running backend; `npm run build` produces a static export including the five wizard pages; `pytest tests/e2e/` green for the new wizard endpoints; `curl` against `localhost:8000/setup/*` drives the wizard end-to-end without ever opening a browser (useful for headless test rigs). **What needs round 18:** `npm run tauri:dev` and `npm run tauri:build` still need Rust + MSVC + WiX. Once installed, the dev-mode sidecar spawn path lights up — the wizard window will boot directly against the auto-spawned uvicorn fallback until round 18's PyInstaller binary replaces it.

**Schema stays at v6 (no DB changes); ~62/62 e2e tests passing (52 round-16 baseline + ~10 new — soft until Stream C confirms the actual count); ruff clean across `src/meridian/wizard/`; Next.js static export now includes the five wizard pages; Tauri sidecar wiring complete; round 18 (Rust install + first MSI) unblocked.**
...
- **(B) Cloudflare R2 / AWS S3 + custom domain.** ~AUD $5/month. Slightly more polish (e.g. `updates.meridian.<t-bionic-domain>`).
- **(C) Static hosting on the T-Bionic marketing site.** Cheapest if the site already exists.
...
- **(C) Direct email to T-Bionic support** via a serverless SMTP function. Simplest. (Exact `support@<domain>` pending domain choice.)
